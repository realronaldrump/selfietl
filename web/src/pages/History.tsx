...
import { api, renderPlaybackUrl, renderPosterUrl, type Project, type Render } from "@/api/client";
  ...
  return (
    <Panel className="grid gap-4 lg:grid-cols-[12rem_1fr_auto] lg:items-center">
      <div className="overflow-hidden rounded-md bg-ink">
        {render.status === "done" ? (
          <video className="aspect-video w-full" src={renderPlaybackUrl(render.id)} poster={renderPosterUrl(render.id)} controls />
        ) : (
